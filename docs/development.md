...
To develop bitcore-node-divi:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-divi.git
...
We now will setup symlinks in `bitcore-node-divi` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-divi
ln -s ~/bitcoind-rpc-divi
```
...
cd devnode
mkdir node_modules
touch bitcore-node-divi.json
touch package.json
```

Edit `bitcore-node-divi.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-divi/bin/bitcore-node-divi start
```