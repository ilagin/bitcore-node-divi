...
Bitcoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  ...
  if (balance) {
    ...
    this.client.getBalance({addresses: addresses}, function(err, response) {
      if (err) {
        return callback(self._wrapRPCError(err));
      }
...
Bitcoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  ...
  function finish(mempoolDeltas) {
    if (utxos) {
      ...
      self.client.listUnspent({addresses: addresses}, function(err, response) {
        if (err) {
          return callback(self._wrapRPCError(err));
        }
  ...
  if (queryMempool) {
    self.client.getChainTips(function(err, response) {
      if (err) {
        return callback(self._wrapRPCError(err));
      }
...
Bitcoin.prototype.getAddressTxids = function(addressArg, options, callback) {
  ...
  function finish() {
    if (txids && !rangeQuery) {
      ...
      self.client.listTransactions(txidOpts, function(err, response) {
        if (err) {
          return callback(self._wrapRPCError(err));
        }
...
Bitcoin.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
  self.client.getBlockHash(high, low, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
...
Bitcoin.prototype.getRawTransaction = function(txid, callback) {
  ...
  if (tx) {
    ...
    var hex = self.rawTransactionCache.get(tx.hex);
    self._tryAllClients(function(client, done) {
      client.decodeRawTransaction(hex, function(err, response) {
        if (err) {
          return done(self._wrapRPCError(err));
        }
        var buffer = new Buffer(response.result, 'hex');
        console.log(`buffer: ${buffer}`);
        self.rawTransactionCache.set(hex, buffer);
        done(null, buffer);
      });
    }, callback);
  }
...
Bitcoin.prototype.getTransaction = function(txid, callback) {
  ...
  if (tx) {
    ...
    self._tryAllClients(function(client, done) {
      client.decodeRawTransaction(txid, function(err, response) {
        if (err) {
          return done(self._wrapRPCError(err));
        }
        var tx = Transaction();
  ...
  if (tx) {
    ...
    self._tryAllClients(function(client, done) {
      client.getRawTransaction(txid, 1, function(err, response) {
        ...
        console.log(`tx hex: ${JSON.stringify(tx.hex)}`)

        client.decodeRawTransaction(tx.hex, function(err, res) {
          if (err) {
            return done(self._wrapRPCError(err))
          }
          console.log(`decoderaw response: ${JSON.stringify(res.result)}`);
        });
        ...
        self.transactionDetailedCache.set(txid, tx.hex, tx);
...
Bitcoin.prototype.generateBlock = function(bool, callback) {
  var self = this;
  this.client.setgenerate(bool, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
