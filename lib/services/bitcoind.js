 ...
 * @param {Function} callback
 */
Bitcoin.prototype.getRawTransaction = function(hex, callback) {
  var self = this;
  ...
  if (tx) {
    ...
    self._tryAllClients(function(client, done) {
      client.getRawTransaction(txid, 1, function(err, response) {
        ...
        client.decodeRawTransaction(tx.hex, function(err, response) {
          if (err) {
            return done(self._wrapRPCError(err))
          }
          console.log(`decoderaw response: ${JSON.stringify(response)}`);
        });
